...
export function AuthButton() {
  ...
  function openModal(m: ModalMode = "login") {
    setMode(m);
    setError(null);
    setUsername("");
    setEmail("");
  ...
  const modal = modalOpen && mounted && createPortal(
    <>
      <div className="fixed inset-0 z-[100] bg-black/60" onClick={() => setModalOpen(false)} />
      <div
        ...
        <div className="flex items-center justify-between mb-5">
          <div className="flex gap-3">
            {mode !== "forgot" && mode !== "forgot-sent" ? (
              ...
              <span className="text-sm font-medium text-slate-100">Reset password</span>
            )}
          </div>
          <button type="button" onClick={() => setModalOpen(false)} className="text-slate-500 hover:text-slate-300 text-lg leading-none">×</button>
        </div>

        {mode === "forgot-sent" ? (
          <div className="space-y-4">
            <p className="text-sm text-slate-300">If that email is registered you&apos;ll receive a reset link shortly. Check your inbox.</p>
            <button
              type="button"
              onClick={() => { setMode("login"); setError(null); }}
              className="w-full rounded-xl border border-slate-600 bg-slate-800 px-4 py-2 text-sm text-slate-100 hover:bg-slate-700 transition-colors"
            >
              Back to sign in
            </button>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            >
              {submitting ? "…" : mode === "login" ? "Sign in" : mode === "register" ? "Create account" : "Send reset link"}
            </button>
            ...
            {mode === "login" && (
              <div className="text-center">
                <button
                  type="button"
                  onClick={() => { setMode("forgot"); setError(null); setEmail(""); }}
