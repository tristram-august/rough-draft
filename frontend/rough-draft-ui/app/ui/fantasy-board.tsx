...
export function FantasyBoard({
  ...
  const colCount = (selectable ? 1 : 0) + 9; // 8 original columns + Status, + checkbox if selectable
          ...
          <table className="w-full sm:min-w-[620px] border-collapse text-sm">
            <thead className="bg-slate-900/60">
              <tr className="text-xs uppercase tracking-wide text-slate-500">
                {selectable && <th className="px-3 py-2.5 text-left font-semibold">Cmp</th>}
                  ...
                  <React.Fragment key={`${row.overall_rank}-${row.player_name}`}>
                    ...
                    <tr className="border-t border-slate-800/60 transition-colors hover:bg-slate-900/40">
                      ...
                      <td className="px-2 py-2">
                        <InjuryBadge injury={row.gsis_id ? injuriesByGsisId.get(row.gsis_id) : null} />
                      </td>
                      <td className="px-2 py-2">
                        <span
